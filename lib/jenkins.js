...
function Jenkins(runner, options) {
  ...
  if(options.reporterOptions) {
    if(options.reporterOptions.junitReportPath){
      junitReportPath = options.reporterOptions.junitReportPath;
    }

    if(options.reporterOptions.junitReportName){
  ...
  runner.on('start', function() {
    ...
    if (reportPath) {
      ...
      console.log("is reportPath is directory: " + isDirectory);
      var currentDate = new Date();
      if (isDirectory) reportPath = path.join(reportPath, suitesName + currentDate.getDate() + "_" + (currentDate.getMonth() + 1) + "_" + currentDate.getFullYear() + ".xml"); 
